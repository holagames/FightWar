	...
	public class JPushBinding : MonoBehaviour
	{
		...
		*/
		...
		public static void setCustomPushNotificationBuilder(string gameObject, string func)
		{
			...
			// 需要根据自己业务情况修改后再调用。
			int builderId = 2;
			string layoutName = "customer_notification_layout";
			// 指定最顶层状态栏小图标
			string statusBarDrawableName = "yourStatusBarDrawableName";
			// 指定下拉状态栏时显示的通知图标
			string layoutIconDrawableName = "yourLayoutIconDrawableName";

			_plugin.Call("setCustomPushNotificationBuilder", gameObject, func, builderId, layoutName,
                statusBarDrawableName, layoutIconDrawableName);
		}
