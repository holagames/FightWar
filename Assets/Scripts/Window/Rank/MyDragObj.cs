...
public class MyDragObj : UIDragDropItem {
    ...
    protected override void OnDragDropRelease(GameObject surface)
    {
        ...
        if (!cloneOnDrag)
        {
                ...
                surface.name == "SpriteAvatarBG12")//成为子物体
            {
                ...
                //rpw.GetComponent<RankPositionWindow>().SetCaptain();
                //rpw.transform.FindChild(surface.name).FindChild("SpriteAvatar").GetComponent<UISprite>().spriteName=
            }
            ...
            {
                //if (GetChildNum() > 1&&mParent.name!=point.name)
                //{
                ...
                //    Debug.LogError(0000);
                //}
                //else if (GetChildNum() == 1 && mParent.name != point.name)
                //{
                //    this.transform.parent = mParent;
                //    this.transform.localPosition = Vector3.zero;
                //    this.transform.localScale = Vector3.one;
                //    Debug.LogError(22222);
                //    //mParent.transform.FindChild(CharacterRoleID.ToString()).GetComponent<BoxCollider>().enabled = true;
                //}
                //else if (mParent.name == point.name) 
                //{
                //    this.transform.parent = point.transform;    //返回
                //    this.transform.localPosition = Vector3.zero;
                //    this.transform.localScale = Vector3.one;
