...
namespace Xft
{
    ...
    public class XftGlowPerObj : MonoBehaviour
    {
        ...
        void PrepareRenderTex()
        {
            if (TempRenderTex == null)
            {
                TempRenderTex = RenderTexture.GetTemporary((int)camera.pixelWidth, (int)camera.pixelHeight, 0);
            }

            if (TempRenderGlow == null)
            {
                TempRenderGlow = RenderTexture.GetTemporary((int)camera.pixelWidth, (int)camera.pixelHeight, 0);
            }
