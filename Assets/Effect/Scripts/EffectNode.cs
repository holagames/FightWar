        ...
        public void SetRenderType(int type)
        {
            ...
            else if (type == 2)
            {
                RenderObj = Owner.GetVertexPool().AddCone(Owner.ConeSize, Owner.ConeSegment,
                    Owner.ConeAngle, Owner.transform.rotation * Owner.OriVelocityAxis, 0, 60f, Owner.UseConeAngleChange, Owner.ConeDeltaAngle);
            }
            else if (type == 3)
            {
                if (Owner.CMesh == null)
                ...
                dir = Owner.transform.rotation * Owner.OriVelocityAxis;
