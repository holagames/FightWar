...
public partial class ReportManager
{
	...
	static string GetBuildTypeFromEditorLog(string editorLogPath)
	{
		...
		foreach (string line in BigFileReader.ReadFile(editorLogPath, BUILD_TYPE_KEY))
		{
	...
	static BuildReportTool.SizePart[] ParseSizePartsFromString(string editorLogPath)
	{
		...
		foreach (string line in BigFileReader.ReadFile(editorLogPath, SIZE_PARTS_KEY))
		{
	...
	static List<BuildReportTool.SizePart> ParseAssetSizesFromEditorLog(string editorLogPath, string[] prefabsUsedInScenes)
	{
		...
		foreach (string line in BigFileReader.ReadFile(_lastEditorLogPath, ASSET_SIZES_KEY))
		{
			if (string.IsNullOrEmpty(line) || line == "\n" || line == "\r\n")
			{
				break;
	...
	static BuildReportTool.SizePart[] GetAllUnusedAssets(string[] scenesIncludedInProject, BuildReportTool.SizePart[] scriptDLLs, string projectAssetsPath, bool includeSvn, bool includeGit, BuildPlatform buildPlatform, bool includeUnusedPrefabs, List<BuildReportTool.SizePart> inOutAllUsedAssets)
	{
		...
		foreach (string fullAssetPath in TraverseDirectory.Do(projectAssetsPath))
		{
			++assetIdx;
			BuildReportWindow.GetValueMessage = "Getting list of used assets " + assetIdx + " ...";
...
*/
	...
	static void ParseDLLs(string editorLogPath, bool wasWebBuild, string buildFilePath, string projectAssetsPath, string editorAppContentsPath, ApiCompatibilityLevel monoLevel, StrippingLevel codeStrippingLevel, out BuildReportTool.SizePart[] includedDLLs, out BuildReportTool.SizePart[] scriptDLLs)
	{
		...
		foreach (string line in BigFileReader.ReadFile(editorLogPath, MONO_DLL_KEY))
		{
			// blank line signifies end of dll list
			if (string.IsNullOrEmpty(line) || line == "\n" || line == "\r\n")
			{
	...
	public static void GetValues(BuildInfo buildInfo, string[] scenesIncludedInProject, string buildFilePath, string projectAssetsPath, string editorAppContentsPath)
	{
		...
		BuildReportWindow.GetValueMessage = "Getting build sizes...";
		...
		foreach (string line in BigFileReader.ReadFile(_lastEditorLogPath, COMPRESSED_BUILD_SIZE_KEY))
		{
