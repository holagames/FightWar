...
public class InteractiveClothWrap
{
	...
	{
		LuaScriptMgr.RegisterLib(L, "UnityEngine.InteractiveCloth", typeof(InteractiveCloth), regs, fields, typeof(UnityEngine.Cloth));
	}
	...
	static int AddForceAtPosition(IntPtr L)
	{
		...
		if (count == 4)
		{
			InteractiveCloth obj = LuaScriptMgr.GetUnityObject<InteractiveCloth>(L, 1);
	...
	static int AttachToCollider(IntPtr L)
	{
		...
		{
			InteractiveCloth obj = LuaScriptMgr.GetUnityObject<InteractiveCloth>(L, 1);
			Collider arg0 = LuaScriptMgr.GetUnityObject<Collider>(L, 2);
