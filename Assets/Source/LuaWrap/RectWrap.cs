...
public class RectWrap
{
	...
	static int set_min(IntPtr L)
	{
		...
		Rect obj = (Rect)o;
		obj.min = LuaScriptMgr.GetVector2(L, 3);
		LuaScriptMgr.SetValueObject(L, 1, obj);
		return 0;
	}
	...
	static int set_max(IntPtr L)
	{
		...
		Rect obj = (Rect)o;
		obj.max = LuaScriptMgr.GetVector2(L, 3);
		LuaScriptMgr.SetValueObject(L, 1, obj);
		return 0;
	}
	...
	static int Contains(IntPtr L)
	{
		...
		if (count == 2 && LuaScriptMgr.CheckTypes(L, 1, typeof(Rect), typeof(LuaTable)))
		{
			Rect obj = LuaScriptMgr.GetNetObject<Rect>(L, 1);
		...
		else if (count == 2 && LuaScriptMgr.CheckTypes(L, 1, typeof(Rect), typeof(LuaTable)))
		{
			Rect obj = LuaScriptMgr.GetNetObject<Rect>(L, 1);
	...
	{
		LuaScriptMgr.CheckArgsCount(L, 2);
		Rect obj = (Rect)LuaScriptMgr.GetVarObject(L, 1);
	...
	{
		LuaScriptMgr.CheckArgsCount(L, 2);
		Rect arg0 = (Rect)LuaScriptMgr.GetVarObject(L, 1);
		Rect arg1 = (Rect)LuaScriptMgr.GetVarObject(L, 2);
		bool o = arg0 == arg1;
		LuaScriptMgr.Push(L, o);
		return 1;
	}
